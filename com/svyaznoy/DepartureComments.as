package com.svyaznoy {
	 ...
	 */
	public class DepartureComments extends Comments {
		...
		public function DepartureComments( id:int ) {
			this.id = id;
			...
			countLoader = provider.getDepartureCommentsCount( id );
		...
		override protected function onCount( event:ProviderEvent ):void {
			...
			super.onCount( event );
			if ( count ) {
				listLoader = provider.getDepartureCommentsList( id );
		...
		private function onAddCalled( event:CommentsEvent ):void {
			...
			addLoader = provider.addDepartureComment( id, getMessage() );
		...
		private function onOldCommentsCalled( event:CommentsEvent ):void {
			listLoader = provider.getDepartureCommentsList( id, 10, 0, 0, event.lastCommentID );
