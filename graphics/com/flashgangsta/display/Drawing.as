...
package com.flashgangsta.display {
	...
	public class Drawing {
		...
		public static function drawRectangle( target:Sprite, x:Number, y:Number, width:Number, height:Number, color:Number = 0, alpha:Number = 1, clear:Boolean = false ):void {
		...
		public static function drawDashedLine( target:Sprite, x:Number, y:Number, width:Number, height:Number, thickness:Number = 1, color:uint = 0, alpha:Number = 1, dash:Number = 6, spacing:Number = 6, caps:String = null ):void {
			...
			target.graphics.lineStyle( thickness, color, alpha, false, LineScaleMode.NORMAL, caps );
			
			for ( var i:int = 0; i < segsCount; i++ ) {
				target.graphics.moveTo( xPoint, yPoint );
				target.graphics.lineTo( xPoint + Math.cos( radians ) * dash, yPoint + Math.sin( radians ) * dash );
				xPoint += xDelta;
				yPoint += yDelta;
			}
			...
			if( delta > dash ) {
				...
				target.graphics.lineTo( xPoint + Math.cos( radians ) * delta, yPoint + Math.sin( radians ) * delta );
			}
			
			target.graphics.moveTo( xEndPoint, yEndPoint );
		...
		public static function drawWedge( target:Sprite, x:Number, y:Number, radius:Number, startAngle:Number, arc:Number, yRadius:Number = 0, alpha:Number = 1, color:uint = 0 ):void {
